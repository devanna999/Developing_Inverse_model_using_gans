...
#from project_required_functions import *
...
def consolidate_papaya_minkowskis():
    DIR = './shapes/papaya_out'
    createFolder('./shapes/papaya_consolidated')
    n_shapes= len(os.listdir(DIR))
    allnames = os.listdir(DIR)
    allnames.sort()
    ...
    np.savetxt('./shapes/papaya_consolidated/W_020',W020,delimiter=' ')
    np.savetxt('./shapes/papaya_consolidated/W_102',W102,delimiter=' ')
    np.savetxt('./shapes/papaya_consolidated/W_120',W120,delimiter=' ')
    np.savetxt('./shapes/papaya_consolidated/W_220',W220,delimiter=' ')
    np.savetxt('./shapes/papaya_consolidated/W_211',W211,delimiter=' ')
...
## from here, are the functions created by me
    
    
    
def save_checkpoint(state,epoch):
    filename='my_checkpoints'+str(epoch)+'.pth.tar'
    print('=>saving checkpoint')
    torch.save(state,filename)
def load_checkpoint(checkpoint,netG,netD,optimizerD,optimizerG):
...
def statistics (image):
    ...
    segmented_image = (img > threshold_global_otsu).astype(np.int8)## earlier it was greater than and equals symbol, making it strictly           greater than worked too good
...
def statistics_train(segmented_image):
    ...
    try:
        ...
        white_pixels=torch.sum(image==1)
        black_pixels=torch.sum(image==0)
    porousity=black_pixels/(white_pixels+black_pixels)
    return porousity

def plot_Array_generated_images(num_of_images,batch_size,nz,device):
    
    fig,axes=plt.subplots(num_of_images//3+1,3,figsize=(10,10),constrained_layout=True)
    fig.suptitle('Array of Generated images')
    count=0
    for i in range((num_of_images//3)+1):
        for j in range(3):
            generated_image,porousity=filter_generated_image(batch_size,nz)
            count+=1
            axes[i,j].imshow(generated_image,cmap='gray')
            axes[i,j].set_title('Generated_image'+str(_count))
...
def statistics_plot(stats_array):
# set a grey background (use sns.set_theme() if seaborn version 0.11.0 or above) 
    sns.set(style="darkgrid")
    sns.histplot(stats_array)
    plt.xlabel('area ratio of porous images')
    plt.ylabel('The count of images for given porousit')
    plt.show()
    print('The mean porousity of the images is {}'.format(np.mean(stats_array)))
    print('The standard deviation of the images is {}'.format(np.std(stats_array)))
def contour_extractor(img):## returns the x and y co-ordinates of the boundaries of the image with origin at left bottom.(img_matrix has different reference)
    ...
    return x,y   
def contour_plot_real_and_extracted(contours,img): ## This is for comparing the input original image and extracted boundaries; It plots original and extracted image side by side.
    imgray=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    ret,thresh=cv2.threshold(imgray,0,1,cv2.THRESH_BINARY)
    contours,hierarchy=cv2.findContours(thresh,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
    ...
    y=500-y ## to account for the difference in imshow image pixels matrix and numpy plots
    ...
    axis[0].plot(x,y)
    axis[1].imshow(img[:,:,0],cmap='gray')
    print('The area covering is ',np.trapz(y,x))
    print('The porousity is ',(np.trapz(y,x)/(500*500)))
    print('the original image shape is ',img[:,:,0].shape)
    ## i can clearly see 90 degree rotation
    #porousity=statistics(imgray)
    #print('The porousity of original image is ',porousity)
def simulate_flow(dp,dir,name):
    #DIR = './shapes/coords'
    DIR=dir
#    dp_0 = 0.000001
...
def average_velocities(name):
    ...
    for k, directory in enumerate(DIR):
        folders= os.listdir(directory)## this should be never used again
...
#                count += 1
           V_mean[:,k+1] = -0.00001
            ...
            flow_data = np.loadtxt(os.path.join(directory+'/'+folder,'xprof'),delimiter=' ')
            V_mean[0,k+1] = np.mean(flow_data[:,6])
        
    
    np.savetxt('flow_mean_velocity',V_mean,delimiter=' ', header=header_text + name )
def compute_permeabilities(mu,points,order=2):
    vol_frac = 0.5
    radius_cyl = np.sqrt(vol_frac/np.pi)
    rho = 1000
    mu = 0.001
    L =radius_cyl 
    vel_mean = np.loadtxt('flow_mean_velocity',delimiter=' ').reshape(1,-1)
    x = vel_mean[:,0:points] # 11110 x 3
    num_shapes = np.shape(x)[0]
    print('the num_shapes is ',num_shapes)
    DIR_ = glob.glob('./simulations_dP_*')
    ...
    print('im inside compure',x)
    id_max = np.argmax(x[:,points-1])
    ...
    for k in range(num_shapes):
        ...
        p_all.append(np.linalg.lstsq(XX[k], y, rcond=None)[0])
    ...
    p_k = np.array(p_all)
    print(p_k[test,:])
    y_ls = np.polyval(p_all[test],x_points)  
    ##change is made here
    pl.figure()
    pl.plot(x_points_nd, y_ls, 'k-')
    
    pl.plot(x_nd[test,0:points],y[0:points], 'r.',markersize=15)
    perm_lst=p_k[:,1]
    np.savetxt('fit_coeffs_order_'+str(order)+'_'+str(points),p_k)
    return mu/(rho*perm_lst)
## combining all the functions to make a function to calculate permeability
# it should take coords as input and do everything in the loopd
def id_generator(size=6, chars=string.ascii_uppercase+string.digits):
    return ''.join(random.choice(chars) for _ in range(size))


def save_random(gen_coords):
    name=id_generator()
    coord_file='./shapes/coords_gen/'+name
    np.savetxt(coord_file,gen_coords,delimiter=' ')
    return name
def permeability_finder(gen_coords,mu,dp_lst,points):
    name=save_random(gen_coords)
    for dp in dp_lst:
        
        simulate_flow(dp,'./shapes/coords_gen',name)
    average_velocities(name)
    permeability=compute_permeabilities(mu,points,order=2)
    return permeability
